...
import { Plus, Pencil, Trash2, X, Ruler, Upload, Download } from 'lucide-react'
...
export default function TreatmentSuppliersPage() {
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      fd.append('file', file)
      const res = await api.post('/suppliers/import-csv', fd, {
  ...
  const downloadTemplate = async () => {
    try {
      const res = await api.get('/suppliers/csv-template', { responseType: 'blob' })
      const url = URL.createObjectURL(res.data as Blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'fornitori_esterni_modello.csv'
      ...
      actions={
          <div className="flex items-center gap-3">
            ...
            <Button size="sm" variant="outline" disabled={importing}
                    ...
                    title="Importa un CSV di fornitori esterni (separatore ;)">
          ...
          <table className="table-fixed w-full text-sm">
            <thead className="bg-muted border-b">
              <tr>
                ...
                <th className="text-left p-3 w-[16%] font-medium text-muted-foreground">Tipo</th>
            ...
            <tbody>
              {visible.length === 0 ? (
                ...
                <tr key={s.id} className="border-b hover:bg-muted">
                  ...
                  <td className="p-3 text-muted-foreground">{s.supplier_type || '—'}</td>
